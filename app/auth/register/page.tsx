...
const RegisterPage = () => {
  ...
  const onSubmit = async () => {
    ...
    const response = await AddUser(email, firstName, lastName, password, "client");
    if (response === 1) {
    ...
    <div>
      ...
      <div className="flex flex-col items-start w-[35%] gap-y-2">
        ...
        <Input
          ...
          className="rounded-[50px] px-[18px] py-[10px] text-lg"
          suffix={<span onClick={()=>setShowPassword(!showPassword)} className="cursor-pointer">{showPassword ? <Eye/> : <EyeOff/>}</span>}
          value={password}
      ...
      <div className="flex flex-col items-start w-[35%] gap-y-2">
        ...
        <Input
          ...
          className="rounded-[50px] px-[18px] py-[10px] text-lg"
          suffix={<span onClick={()=>setShowConfirm(!showConfirm)} className="cursor-pointer">{showConfirm ? <Eye/> : <EyeOff/>}</span>}
      ...
      <div className={`${successColor} font-medium text-[18px] w-[30%] text-center`}>
        <p className="text-center text-md tracking-tight font-semibold text-red-400">{message}</p>
        {success} 
